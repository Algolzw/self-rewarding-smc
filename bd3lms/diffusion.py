...
#   return torch.distributions.Categorical(probs=categorical_probs).sample()
...
class Diffusion(L.LightningModule):
  ...
  #######################################################
  ########### Updated for self-rewarding SMC#############
  #######################################################
  @torch.no_grad()
  def _ddpm_caching_update(self, x, t, dt, cache=None):
    ...
    if cache is None:
      if self.config.sampling.kv_cache:
        logp_x0 = self.forward(x[:, -self.block_size:],
                        sigma_t,
                        sample_mode=True).to(torch.float64)
      else:   
        logp_x0 = self.forward(x,
                          sigma_t,
                          sample_mode=True).to(torch.float64)
        logp_x0 = logp_x0[:, -self.block_size:]

      p_x0 = self._nucleus_sample(logp_x0.exp())
    else:
      p_x0, logp_x0 = cache
    ...
    if self.config.sampling.first_hitting:
      ...
      ################################################################
      ### updated code for batched logp
      mask_bool = (x[:, -self.block_size:] == self.mask_index)          # [B, S]
      num_masked = mask_bool.sum(-1)                                    # [B]
      ind0 = (torch.rand(x_block.shape[0], device=x.device) * num_masked.clamp(min=1).float()).long()  # [B]
      cumsum = mask_bool.long().cumsum(dim=1)                           # [B, S]
      ind = (cumsum == (ind0[:, None] + 1)).int().argmax(dim=1)         # [B]
      mask = (torch.arange(self.block_size, device=x.device) == ind[:, None]).to(x_block.dtype)  # [B, S]
      x_block = x_block * mask + x[:, -self.block_size:] * (1 - mask)
      logp_x0 = torch.squeeze(
            torch.gather(logp_x0, dim=-1, index=torch.unsqueeze(x_block, -1)), -1) # b, l
      logp_x0 = logp_x0 * mask
      logp_x0 = logp_x0 - torch.log(num_masked.clamp(min=1).to(x_block.dtype)).unsqueeze(1) * mask
      ################################################################
    else:
    ...
    ################################################################
    transfer_mask = (x[:, -self.block_size:] == self.mask_index) & (x_block != self.mask_index)
    ################################################################
    ...
    cache = None if not torch.allclose(x_new, x) else (p_x0, logp_x0)
    return cache, logp_x0, x_new, transfer_mask
  ...
  def _sample(
    ...
    if self.sampler == 'semi_ar':
      for idx in range(self.config.sampling.num_sample_batches):
        ...
        while sample_i is None:
          ...
          sample_i, nfes = self._semi_ar_sampler(
            ...
            num_steps=num_steps,
            seqlen=seqlen,
            use_smc=self.config.sampling.use_smc,
            n_particles=self.config.sampling.n_particles,
            resample_freq=self.config.sampling.resample_freq
          )
          if num_tries > 10:
            raise ValueError('Sampling failed.')
        samples.append(sample_i)
  ...
  def _semi_ar_sampler(
    ...
    use_smc=False, n_particles=4, resample_freq=4):
    if use_smc:
      assert n_samples == 1, "Currently, the batch size for inference must be 1!"
      assert n_particles > 1
      n_samples = n_samples * n_particles
    if seqlen is None:
      seqlen = self.config.model.length
    sampling_steps = 0
    ...
    for stride_num in tqdm(range(num_strides)):
      ...
      for i in range(num_steps):
        ...
        cache, logp_x0, x_next, transfer_mask = self._ddpm_caching_update(
            x=x_accum[:, fwd_idx],
            t=t.unsqueeze(1) * ones,
            dt=dt,
            cache=cache,)
        if cache is None:
          sampling_steps += 1
        ...
        ################################################################
        # SMC Resampling
        if use_smc and n_particles > 1:
          # weighting, seqlen
          # beta_w = i / num_steps # 26.01
          beta_w = sampling_steps / seqlen # 21.40
          # beta_w += 1 / seqlen
          log_w = log_w + beta_w * (logp_x0 * transfer_mask.float()).sum(dim=1) # weight on transfer tokens
          weights = torch.exp(log_w - log_w.max())
          weights = weights / weights.sum()
          # resampling
          ess = 1.0 / (weights.pow(2).sum())
          if sampling_steps % resample_freq == 0 and ess < 0.5 * n_particles:
            # print(f"Resampling at sampling_steps {sampling_steps}, with ess: {ess:.2f}, beta_w: {beta_w:.4f}")
            k_idx = torch.multinomial(weights, num_samples=n_particles, replacement=True).squeeze(-1)
            x_accum = x_accum[k_idx]; log_w.zero_()
        ################################################################
        # x_accum[:, fwd_idx] = x_next

      # check if we need to resample (or stop sampling for variable-length sampling)
      if x_accum.shape[1] > 256:
        stop, x_accum[:1] = self._check_stop_conds(x_accum[:1])
        if (stop and not self.config.sampling.var_length) \
          or (stop and x.shape[-1] == 1):
          # print("entropy is low.")
          return None, None
        elif stop:
          break
    if use_smc:
        sampling_steps *= n_particles
    return x_accum[:1], sampling_steps
